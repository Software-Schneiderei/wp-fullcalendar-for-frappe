...
(() => {
/*!*********************************************!*\
  !*** ./src/fullcalendar-for-frappe/view.js ***!
  \*********************************************/
__webpack_require__.r(__webpack_exports__);
    ...
    const staticCalendarOptions = {
      ...
      events: {
        ...
        success(response) {
          ...
          Array.prototype.slice.call(events).forEach(event => event.title = pluralize(event.anzahl, ' Termin verfügbar', ' Termine verfügbar'));
          return events;
        }
      }
    };
    root.appendChild(calendarEl);
    const customCalendarOptions = JSON.parse(root.getAttribute('data-calendar-options') || '{}');
    const calendarOptions = {
      ...staticCalendarOptions,
      ...customCalendarOptions
    };
    const calendar = new _fullcalendar_core__WEBPACK_IMPORTED_MODULE_0__.Calendar(calendarEl, calendarOptions);
    calendar.render();
